						...
						<div class="beta-products-list">
							...
							@foreach($sp_khac as $khac)
								<div class="col-sm-4">
									...
									@endif
										<div class="single-item-header">
											<a href="chi-tiet-san-pham/{{$khac->id}}"><img src="source/image/product/{{$khac->image}}" alt="" height="250px"></a>
										</div>
										<div class="single-item-body">
											<p class="single-item-title">{{$khac->name}}</p>
											<p class="single-item-price">
											@if($khac->promotion_price == 0)
												<span class="flash-sale">{{number_format($khac->unit_price)}} Đ</span>
												
											@else
											    <span class="flash-del">{{number_format($khac->unit_price)}} Đ</span>
												<span class="flash-sale">{{number_format($khac->promotion_price)}} Đ</span>

											@endif
											</p>
										</div>
										<div class="single-item-caption">
											<a class="add-to-cart pull-left" href="add-to-cart/{{$khac->id}}"><i class="fa fa-shopping-cart"></i></a>
											<a class="beta-btn primary" href="chi-tiet-san-pham/{{$khac->id}}">Details <i class="fa fa-chevron-right"></i></a>
											<div class="clearfix"></div>
										</div>
									</div>
								</div>
